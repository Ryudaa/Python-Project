...
def stwo(vertex, g):
    ...
    for c in comb:
        ...
        s_i = "S"+str(index)+"={"+str(c[0])+","+str(c[1])+"}"
        s_string = "S("+str(c[0])+","+str(c[1])+")"
        r_v_s_i = "r("+str(c[0])+"|S"+str(index)+")={0,"+jarak_baris_kolom+"}"
        #print(s_i)
        #print(r_v_s_i)
        
        list_s_i.append(s_i)
        list_r.append(r_v_s_i)

        for i in range(0, jmlh_vertex):
            r_vj_s_i = "r("+str(vertex[i])+"|"+s_string+")={"+isi_table[i][baris]+","+isi_table[i][kolom]+"}"
            #cek kiri
            jumlah_sama = 0
            for k in comb2:
                if isi_table[k[0]][baris] == isi_table[k[1]][baris]:
                    if isi_table[k[0]][kolom] == isi_table[k[1]][kolom]:
                        jumlah_sama += 1
                
            if (jumlah_sama == 0):
                r_vj_s.append(str(r_vj_s_i)+" Y")
                list_v_memenuhi.append(str("r("+str(vertex[i])+"|"+s_string+")"))
            else:
                r_vj_s.append(str(r_vj_s_i)+" N")
